...
function DialogShell({ descriptor, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-flame-900/60 backdrop-blur-sm"
      ...
      >
        <button
          onClick={() => onClose(null)}
          aria-label="Close"
          className="absolute top-3 right-3 w-8 h-8 rounded-full text-flame-500 hover:bg-cream-100 flex items-center justify-center transition"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="flex items-start gap-3">
          {descriptor.destructive && (
            <span className="w-9 h-9 rounded-xl bg-flame-500 text-white flex items-center justify-center shrink-0">
              <AlertTriangle className="w-5 h-5" />
            </span>
          )}
          <div className="min-w-0 flex-1 pr-6">
            <h2 id={`dlg-title-${descriptor.id}`} className="text-lg font-bold text-flame-900">
              {descriptor.title}
            </h2>
            {descriptor.message && (
              <p className="mt-1 text-sm text-flame-600 whitespace-pre-wrap">{descriptor.message}</p>
            )}
          </div>
        </div>
        ...
        {kind === 'prompt' && (
          <div className="mt-4">
            <input
              ...
              className="input"
            />
            {error && <p className="mt-1.5 text-xs text-flame-700">{error}</p>}
          </div>
        )}
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          ...
          <button
            onClick={submit}
            className={descriptor.destructive ? 'btn-primary text-sm bg-flame-900 hover:bg-flame-800 focus:ring-flame-900/40' : 'btn-primary text-sm'}
          >
